...
from retrieval_dataset_neg import (
...
class CombinedRetrievalHardNegLoss(nn.Module):
    def __init__(self, temperature: float = 0.07, hard_neg_margin: float = 0.10, hard_neg_weight: float = 0.30):
        super().__init__()
        self.base_loss = MultiPositiveCrossModalLoss(temperature=temperature)
        self.hard_neg_margin = hard_neg_margin
        self.hard_neg_weight = hard_neg_weight

    def forward(self, query_emb, target_emb, tm_ids, negative_emb=None, has_negative=None):
        base = self.base_loss(query_emb, target_emb, tm_ids)

        if negative_emb is None or has_negative is None:
            return base

        mask = has_negative > 0.5
        if mask.sum() == 0:
            return base

        pos_scores = (query_emb[mask] * target_emb[mask]).sum(dim=1)
        neg_scores = (query_emb[mask] * negative_emb[mask]).sum(dim=1)

        hard_loss = F.relu(self.hard_neg_margin + neg_scores - pos_scores).mean()
        return base + self.hard_neg_weight * hard_loss
...
def compute_retrieval_metrics_and_rows(sims, query_meta, gallery_meta, ks=(1, 5, 10, 20)):
    ks = tuple(sorted(set(int(k) for k in ks if int(k) > 0)))
    if not ks:
    ...
    for q_idx in range(sims.shape[0]):
        ...
        for k in ks:
            hit = int(any(tm == q_tm_id for tm in top_tm_ids[:min(k, len(top_tm_ids))]))
            row[f"hit_at_{k}"] = hit
            hit_counts[k] += hit
    ...
    for k in ks:
        metrics[f"recall_at_{k}"] = hit_counts[k] / num_queries
...
def evaluate_retrieval_on_fold(model, fold_dir: Path, project_root: Path, device, batch_size: int, num_workers: int, image_size: int):
    ...
    metrics, topk_df = compute_retrieval_metrics_and_rows(
        sims=sims,
...
def train_one_fold(
    ...
    criterion = CombinedRetrievalHardNegLoss(
    ...
    if not freeze_backbone:
        param_groups.insert(0, {"params": model.backbone.parameters(), "lr": lr_backbone})
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            ...
            loss = criterion(query_emb, target_emb, tm_ids, negative_emb=negative_emb, has_negative=has_negative,)
        ...
        row = {
            ...
            "val_recall_at_10": val_metrics["recall_at_10"],
        ...
        if current_score > best_score:
            ...
            best_metrics = {
                ...
                "best_val_recall_at_10": val_metrics["recall_at_10"],
                "best_val_recall_at_20": val_metrics["recall_at_20"],
                "num_queries": val_metrics["num_queries"],
                "gallery_size": val_metrics["gallery_size"],
...
def main():
    ...
    parser.add_argument("--hard_negatives_csv", type=str, default=None)
    parser.add_argument("--use_hard_negatives", action="store_true")
    parser.add_argument("--hard_neg_margin", type=float, default=0.10)
    parser.add_argument("--hard_neg_weight", type=float, default=0.30)
    ...
    print(f"Use HN:       {args.use_hard_negatives}")
    print(f"HN csv:       {args.hard_negatives_csv}")
    ...
    for fold_dir in fold_dirs:
        best_metrics = train_one_fold(
            ...
            hard_negatives_csv=args.hard_negatives_csv,
    ...
    for k in (1, 5, 10, 20):
        col = f"best_val_recall_at_{k}"
        summary[f"mean_best_recall_at_{k}"] = float(all_best_df[col].mean())
        summary[f"std_best_recall_at_{k}"] = float(all_best_df[col].std(ddof=0))
